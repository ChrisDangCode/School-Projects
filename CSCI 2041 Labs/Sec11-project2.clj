...
; Name: Chris Dang
; Created: Nov. 27, 2016
; Level 3 Perjure interpreter
;===============================================================================

(declare evaluate-letfn atomize)

; Based on Perjure level 2 from moodle
...
; LETFN-CALL - determines if form is a call to the special form letfn
(def letfn-call?
 (fn [form]
  (and
   (not (empty? form))
...
(def atomize
  (fn [form]
      (loop [values (vals (first form))]
        (if (empty? values)
          nil
          (do
            (reset! (get (first values) 3) form)
            (recur (rest values)))))))
...
(def evaluate-letfn
 (fn [form scope]
  (loop
   ...
   (if
    (empty? pairs)
     (let [name (atomize (cons map scope))]
       (evaluate
         (first (rest (rest form)))
          (cons map scope)))
    (recur
